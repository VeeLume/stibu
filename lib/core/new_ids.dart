...
import 'package:stibu/models/expenses.dart';
import 'package:stibu/models/invoices.dart';
import 'package:watch_it/watch_it.dart';

Future<Result<int, AppwriteException>> newCustomerId() async {
  ...
  try {
    final doc = await appwrite.databases.listDocuments(
      ...
      queries: [Query.orderDesc('id'), Query.limit(1)],
    );
...
Future<Result<String, AppwriteException>> newExpenseNumber([
  DateTime? date,
]) async {
  ...
  try {
    final latestExpense = await appwrite.databases.listDocuments(
      ...
      queries: [Query.orderDesc('expenseNumber'), Query.limit(1)],
    );
...
Future<Result<String, AppwriteException>> newInvoiceNumber([
  DateTime? date,
]) async {
  final utcDate = date?.toUtc() ?? DateTime.now().toUtc();
  final appwrite = di<AppwriteClient>();
  try {
    final latestInvoices = await appwrite.databases.listDocuments(
      ...
      queries: [Query.orderDesc('invoiceNumber'), Query.limit(1)],
    );
