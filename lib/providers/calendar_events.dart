...
import 'package:stibu/models/calendar_events.dart' as _i3;
...
class CalendarEventsProvider extends _i1.ChangeNotifier
    implements _i2.GenericListProvider<_i3.CalendarEvents> {
  final List<_i3.CalendarEvents> _items = [];

  int _totalItems = 0;

  @override
  List<_i3.CalendarEvents> get items => [..._items];

  @override
  bool get hasMore => _items.length < _totalItems;

  Future<void> _fetch() async {
    final result = await _i3.CalendarEvents.page(
  ...
  Future<void> build() async {
    ...
    realtimeSubscriptions.subscribe(
      'databases.672bcb590033b5b2780a.collections.672bce09001905da8109.documents',
      (message) {
        final event = message.events.first.split('.').last;
        switch (event) {
          case 'create':
            _items.add(_i3.CalendarEvents.fromAppwrite(
                _i6.Document.fromMap(message.payload)));
            _totalItems++;
          case 'update':
            final newItem = _i3.CalendarEvents.fromAppwrite(
                _i6.Document.fromMap(message.payload));
            final index = _items.indexWhere(
                (_i3.CalendarEvents item) => item.$id == newItem.$id);
            index != -1 ? _items[index] = newItem : null;
          case 'delete':
            final deletedItem = _i3.CalendarEvents.fromAppwrite(
                _i6.Document.fromMap(message.payload));
            _items.removeWhere(
                (_i3.CalendarEvents item) => item.$id == deletedItem.$id);
            _totalItems--;
        }
        notifyListeners();
      },
