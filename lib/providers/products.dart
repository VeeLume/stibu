...
import 'package:stibu/models/products.dart' as _i3;
...
class ProductsProvider extends _i1.ChangeNotifier
    implements _i2.GenericListProvider<_i3.Products> {
  final List<_i3.Products> _items = [];

  int _totalItems = 0;

  @override
  List<_i3.Products> get items => [..._items];

  @override
  bool get hasMore => _items.length < _totalItems;

  Future<void> _fetch() async {
    final result = await _i3.Products.page(
  ...
  Future<void> build() async {
    ...
    realtimeSubscriptions.subscribe(
      'databases.public.collections.products.documents',
      (message) {
        final event = message.events.first.split('.').last;
        switch (event) {
          case 'create':
            _items.add(_i3.Products.fromAppwrite(
                _i6.Document.fromMap(message.payload)));
            _totalItems++;
          case 'update':
            final newItem = _i3.Products.fromAppwrite(
                _i6.Document.fromMap(message.payload));
            final index = _items
                .indexWhere((_i3.Products item) => item.$id == newItem.$id);
            index != -1 ? _items[index] = newItem : null;
          case 'delete':
            final deletedItem = _i3.Products.fromAppwrite(
                _i6.Document.fromMap(message.payload));
            _items.removeWhere(
                (_i3.Products item) => item.$id == deletedItem.$id);
            _totalItems--;
        }
        notifyListeners();
      },
